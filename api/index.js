...
const DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/150.0.0.0 Safari/150.0.0.0";
...
async function initDatabase() {
    try {
        // تحديد مسار ملف الـ wasm محلياً داخل مجلد api ليتوافق مع Vercel
        ...
        console.log('Connected to the SQLite database via sql.js on Vercel.');
    } catch (err) {
...
// ============================================================
// 🔐 دالة جلب الكوكيز (الأولوية: Vercel Env → ملف محلي)
// ============================================================
function getFebboxCookie() {
    // الأولوية: متغيرات البيئة في Vercel
    if (process.env.FEBBOX_COOKIE) {
        return process.env.FEBBOX_COOKIE;
    }

    // الاحتياطي: ملف محلي (للتطوير فقط)
    try {
...
// ============================================================
// 🔐 نقطة نهاية لجلب التوكن (آمنة عبر Vercel Env)
// ============================================================
app.get('/api/token', (req, res) => {
    // قراءة التوكن من متغيرات البيئة في Vercel
    const token = process.env.JWT_TOKEN || process.env.API_TOKEN || null;
...
app.get('/api/config', (req, res) => {
    res.json({
        ...
        api_version: "13.1 (Vercel Anti-Duplicate & Multi-Language Engine)"
    });
});
...
app.get('/api/category', (req, res) => {
    ...
    const excludeIdsParam = (req.query.exclude || "").trim();
...
app.get(['/api/stream', '/api/stream/:movie_id'], async (req, res) => {
    try {
        ...
        // ============================================================
        // 🔐 جلب الكوكيز بدون استخدام global
        // ============================================================
        const userCookie = getFebboxCookie();

        const movieInfo = { tmdb_id: tmdbId, type: type, season: season, episode: episode };
        
        // ============================================================
        // 🔐 تمرير الكوكيز كمعامل إلى wrapper
        // ============================================================
        await wrapper.getResource(movieInfo, {}, userCookie, (streamResult) => {
